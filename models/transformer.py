...
class Attention(nn.Module):
    ...
    def forward(self, x, freqs_cis):
        ...
        if self._log_attn_stats:
            with torch.no_grad():
                _logits = torch.matmul(q, k.transpose(-2, -1)) * (self.head_dim**-0.5)
                _causal = torch.triu(
                    torch.ones(seqlen, seqlen, device=q.device, dtype=torch.bool), diagonal=1
                )
                self._attn_logit_max = _logits.masked_fill_(_causal, float("-inf")).max().item()
