...
class TorchEngine(torch.nn.Module):
    ...
    def __init__(self, model, cfg, device):
        ...
        # Mixed-precision invariant: when training in bf16, master parameters
        # MUST be fp32. We rely on `torch.amp.autocast` (below) to cast the
        # forward pass to bf16 on the fly while keeping fp32 master weights,
        # fp32 grad buffers, and fp32 optimizer state. If `model` arrives in
        # bf16 (e.g. Hugging Face's `from_config` honoring a `dtype: bfloat16`
        # field on the published config), autocast is a no-op, optimizer state
        # is bf16, and weight updates are quantized to the bf16 grid.
        if self.dtype == "bfloat16":
            master_dtypes = {p.dtype for p in self.model.parameters()}
            assert master_dtypes == {torch.float32}, (
                f"Engine expects fp32 master weights for bf16 mixed-precision "
                f"training; got dtypes {master_dtypes}. Cast the model to fp32 "
                f"before passing it in (e.g. `model = model.float()` after "
                f"construction), or switch to a pure-bf16 training recipe."
            )
        ...
        if cfg.resume:
            ...
            for name, optim in self.optimizers.items():
                if getattr(optim, "rank_sharded", False):
    ...
    def step(self, batch):
        ...
        if (
            self.abort_on_bad_loss
            and loss_val > MAX_ALLOWED_LOSS
            and self.steps > MIN_STEPS_BEFORE_CHECK
        ):
            raise ValueError(
                f"Train loss {loss_val} exceeds {MAX_ALLOWED_LOSS} at step {self.steps}."
            )
