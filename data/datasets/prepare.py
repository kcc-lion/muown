...
flags.DEFINE_string(
    "out_path",
    "/fast/najroldi/data/lm/fwedu/test",
...
flags.DEFINE_boolean(
    "streaming",
    False,
    "Download the dataset in streaming mode. Ignored if `download` is False.",
)
...
flags.DEFINE_integer(
    "seq_length",
    None,
    "Sequence length for chunking the dataset. Ignored if `chunk` is False.",
)

flags.DEFINE_boolean(
    "split_train_valid",
    True,
    "Split the dataset into train and valid sets after chunking.",
)
flags.DEFINE_integer(
    "n_tokens_valid",
    None,
    "Number of tokens in the validation set. Ignored if `chunk` is False.",
)

flags.DEFINE_boolean(
    "save_raw",
    False,
    "Save the raw dataset to disk. Ignored if `download` is False.",
)
flags.DEFINE_boolean(
    "save_tokenized",
    False,
    "Save the tokenized dataset to disk. Ignored if `tokenize` is False.",
)
flags.DEFINE_boolean(
    "save_tokenizer",
    False,
    "Save the tokenizer to disk. Ignored if `tokenize` is False.",
)
...
def tokenize_batched(examples, tokenizer):
    # Some tokenizers (e.g. Qwen2) do not define a BOS token; treat missing
    # special tokens as empty strings so concatenation does not fail.
...
def main(_):
    ...
    map_setup = dict(batched=True, batch_size=1024, num_proc=48)
    ...
    if FLAGS.tokenize:
        ...
        if FLAGS.save_tokenized:
            if os.path.exists(
                os.path.join(out_path, f"tokenized_{tokenizer_name}", "tokenized_dataset")
            ):
        ...
        if FLAGS.save_tokenizer:
            ...
            tokenizer.save_pretrained(
                os.path.join(out_path, f"tokenized_{tokenizer_name}", "tokenizer")
            )
    ...
    if not FLAGS.split_train_valid:
        chunked_ds.save_to_disk(
            os.path.join(out_path, f"ctx_{FLAGS.seq_length}", FLAGS.dataset_split)
        )
        ...
        train_ds_path = os.path.join(
            ...
            f"ctx_{FLAGS.seq_length}",
            "train",
        )
        valid_ds_path = os.path.join(
            out_path,
            f"tokenized_{tokenizer_name}",
            f"ctx_{FLAGS.seq_length}",
            "valid",
